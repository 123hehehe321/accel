...
- **版本**:v0.2 / **2.5-D7 fix7-static 完成,生产部署 + 长跑观察阶段**
- **支持算法**:`accel_cubic`(基线) + `accel_brutal`(激进) + **`accel_smart`(2.5 新增,纯丢包自适应)**
- **代码量**:~3850 行 / 3000 预算(用户拍板放宽,2.6+ 严控)
- **构建方式**:**全静态(glibc + libelf + libbpf + zlib + zstd 全嵌入)**,跑 `./build-static.sh`,详见 §5.15。任何 x86_64 Linux ≥ 6.4 都能跑,无 glibc 版本依赖
- **当前源分支 tip**:`claude/accel-smart` (fix7-static,本次提交)
- **当前 binaries tip**:fix7-static / accel md5 `60637b4dacea4b98735733f97aaf10b0` (3037064 bytes)
...
| fix7-static | fix7 binary 在 Debian 12 VPS 上 glibc 2.39 not found,systemd restart-loop | `.cargo/config.toml` + `build-static.sh` 切全静态构建,binary 自带 glibc/libelf/libbpf/zlib/zstd,任何 x86_64 Linux 都能跑 |
...
### 5.15 binary 必须全静态链接(fix7-static)

**铁律:每个发到 binaries 分支的 binary 都全静态 —— 不依赖任何系统 .so**。

历史教训(fix7 binary 翻车):dev VM 是 Ubuntu 24.04 / glibc 2.39,默认
`cargo build --release` 出来的 binary 链接 GLIBC_2.39 弱符号
(`pidfd_getpid` / `pidfd_spawnp`,Rust 1.94 std 引入)。VPS 是 Debian 12
/ glibc 2.36 → ld.so 报 "version `GLIBC_2.39' not found" → systemd
restart-loop 100+ 次,服务完全跑不起来。

**根因不是内核**:用户 VPS 内核 `6.12.74` 完全 OK。**glibc 是用户态的 C
标准库**,前向不兼容(高版本 build 不能跑在低版本系统上)。内核版本跟
glibc 版本是两回事。

**解决方案** = 全静态构建:
1. `.cargo/config.toml` 锁死 target = `x86_64-unknown-linux-gnu` +
   rustflags 包含 `-C target-feature=+crt-static -C link-arg=-static
   -C link-arg=-l:libelf.a -C link-arg=-l:libz.a -C link-arg=-l:libzstd.a`
2. host 装齐 `libelf-dev zlib1g-dev libzstd-dev linux-libc-dev`
3. 项目根目录的 `build-static.sh` = 一键 build + 验证(`file` /
   `ldd` / `objdump` 三重)
4. **每次发 binary 都跑这个脚本**,它失败就别 push

**为什么不走 musl**:试过。Ubuntu apt 的 `libelf-dev` 只有 glibc-ABI
头文件+静态归档,musl 的 `elf.h` 缺 `Elf64_Relr` typedef → libelf 编译
失败,且 apt 没有 musl-libelf 替代包。glibc 静态对我们够了:不用 DNS /
NSS / locale,所有 syscall 直走内核。

**Rust `+crt-static` 不够**:它只静态链 CRT 启动代码,libc 主体仍动态。
要真全静态:`+crt-static` + `-C link-arg=-static` + 显式 `-l:libxxx.a`。

**通用教训**:任何"对外分发的 production binary"都该静态链接,不依赖
目标机的 system libc。dev/prod 永远不会保持版本一致,别赌。

**新会话约定**:看到"build accel binary 准备发 binaries 分支"任务,
**必须**用 `./build-static.sh`,验证三项全过才 push。
...
1. 读这份 PROJECT_CONTEXT.md(**尤其 §5.15 静态构建铁律**)
2. 读 ebpf/algorithms/VENDOR.md
3. 读 src/ 主要文件了解代码结构
4. 等架构师指令(用户转发架构师消息)
5. 撞坑停下汇报,**不自作主张**
6. 涉及生产 binary:**必须用 `./build-static.sh`**,绝不用 `cargo build --release` 直接发 binary。脚本会自动 `file` / `ldd` / `objdump` 三重验证,失败就别 push。fix7 那次直接 cargo build 出来的 binary 链 GLIBC_2.39 把用户 VPS 整成 systemd restart-loop 100+ 次
7. host 没装 `libelf-dev zlib1g-dev libzstd-dev linux-libc-dev`?先 `apt install`,否则静态 build 链接失败
...
**最后更新**:2026-04-27(2.5-D7 fix7-static 收官)
