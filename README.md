...
**当前版本**: v0.2,**2.5 D1-D7 完成** (2026-04),等真业务流量反馈
...
| **2.5** | accel_smart(自适应 + tc-bpf 多倍发包)| 累计 ~3700 | ✅ D1-D7 完成 | 跨境真业务流量观察 |

### 6.2 当前阶段

**当前**:2.5 D7 完成,等真跨境业务流量反馈。`./accel status` 看 smart state 分布合理性。
...
分类信号:`loss_ewma_bp` 5 秒窗口 EWMA + `srtt/min_rtt` 比例 + 200ms 滞后区。
...
rtt_congest_pct = 50             # 默认 50% RTT 膨胀 → CONGEST
```
...
3. 解析 `[smart]` 段,验证 rate_mbps + 解析 ifindex + 解析端口范围
4. 写 `smart_config_map` 和 `smart_dup_config`
...
### 12.11 LOSSY 在零 RTT 链路被分类成 CONGEST

D6 集成测试用 netns + veth(baseline RTT ≈ 50µs)发现:5% 丢包导致
TCP RTO(最小 200ms),srtt 飙到 ~100ms,`srtt/min_rtt` 比例飙到 2000×,
直接触发 `rtt_congest_pct >= 50%` → CONGEST。LOSSY 在 veth 测不出。

**这不是算法 bug,是测试环境扭曲**。真链路(100ms+ baseline RTT)上,
5% 丢包导致的 srtt 抖动相对小,`srtt/min_rtt ≈ 1.1-1.5`,EWMA 平均下来
不一定过 50% 阈值,LOSSY 能正常分类。详见 PROJECT_CONTEXT §5.11。
