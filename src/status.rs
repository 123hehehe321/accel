...
pub fn render(state: &State) -> String {
    ...
    if let Some(saved) = state.smart_saved.as_ref() {
        ...
        let _ = writeln!(
            s,
            "  smart thresholds:  lossy={}bp congest={}bp",
            saved.loss_lossy_bp, saved.loss_congest_bp
        );
        let _ = writeln!(
            s,
        ...
        let _ = writeln!(
            s,
            "  smart dup factor:  {}x (LOSSY: 1 packet → {} on the wire)",
            saved.duplicate_factor, saved.duplicate_factor
        );
        if saved.port_min > 0 {
            let _ = writeln!(
                s,
